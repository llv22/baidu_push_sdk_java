...
public class CloudHttpClient implements CloudHttpObservable {
	...
	private static Logger logger = Logger.getLogger(CloudHttpClient.class.getName());
	...
	public HttpRestResponse doExecutePostRequestResponse(String surl,
		...
		for (int i = 0; i < maxRetryTimes; i++) {
			...
			try {
				...
				if (i + 1 >= maxRetryTimes) {
					...
					notifyAndCallback(new CloudHttpEvent(
							CloudHttpEvent.TCP_CONNECT_FAIL, surl,
							paramsContent, httpStatusCode,
							"Network connect fail, domain[" + BaiduPushConstants.CHANNEL_REST_URL
							+ "] exists?"));
					
					throw new CloudHttpClientException("Network connect fail, domain[" + BaiduPushConstants.CHANNEL_REST_URL
							+ "] exists?");
			...
			try {
				...
				if (httpStatusCode == 200) {
					...
					if (httpConfigure != null
						...
						if (location != null) {
							...
							// @notify log event
							notifyAndCallback(new CloudHttpEvent(CloudHttpEvent.NETWORK_NORMAL,
				...
				// @notify log event
				notifyAndCallback(new CloudHttpEvent(CloudHttpEvent.NETWORK_IO_EXCEPTION,
		...
		// @notify log event
		notifyAndCallback(new CloudHttpEvent(CloudHttpEvent.NETWORK_TRY_TIMES_EXCEPTION,
				surl, paramsContent, httpStatusCode, responseContent));
		
		throw new CloudHttpClientException("Network connect fail, domain ["
